...
(function() {
  ...
  app.controller('KeywordsController', function ($scope, RepositoryFactory, resolveEntity) {
    ...
    /* == Frontent initialization == */

    /* All of this happens as soon as the page loads */

    /* resolveEntity is a helper function which is used
     * in partials/keywordsCategoryGridCell.html in order
     * to display the name of a keyword category based on id */
    $scope.resolveEntity = resolveEntity;

    /* A repository is the connection between this controller
    and the REST Api, we use one for keyword categories */
    ...
    /* When the frontend loads, we want the controller to
     * immediately load all keyword categories from the api */
    ...
    /* the Grid. */
     $scope.keywordsGridOptions = {
       data: 'keywords', // this make the grid use the data in $scop.keywords
       ...
       columnDefs: [
         ...
         {
           /* the keyword category field does not use the build-in
            * cell template, but our own */
            field: 'keywordsCategoryID',
         ...
         },
         {
           ...
           cellTemplates: 'app/keywords/partials/operationsGridCell.html',
           enableCellEdit: false,
           sortable: false
         }
       ]
     ...
     /* these functions are called when the frontend is
      * operated, e.g. if a button is clicked */
      ...
      /* These are here to make the grid behave cleanly in
       * regards to the keyword category select */
      $scope.stopEditingKeywordsCategory = function() {
        $scope.$broadcast('ngGridEventEndCellEdit');
      };
